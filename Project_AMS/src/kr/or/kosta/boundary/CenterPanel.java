...
public class CenterPanel extends Panel {
	...
	private void eventRegist() {
		...
		searchAllB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (accTypeC.getSelectedItem().equals("전체계좌")) {
					List<Account> allList = accountManager.list();
					if (allList.size() == 0) {
						JOptionPane.showMessageDialog(null, 
								  					  "등록된 계좌 목록이 없습니다.", 
								  					  "주의",
					...
					List<Account> typeList = accountManager.list(accTypeC.getSelectedItem());
					if (typeList.size() == 0) {
						JOptionPane.showMessageDialog(null, 
								  					  "등록된 계좌 목록이 없습니다.", 
								  					  "주의",
		...
		searchAccB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					accListTA.setText(null);
					if(accTypeC.getSelectedItem().equals("전체계좌") || accTypeC.getSelectedItem().equals(account.getAccType())) {
						accListTA.setText(null);
		...
		deleteAccB.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					if((accTypeC.getSelectedItem().equals("전체계좌") || accountManager.isExisting(accTypeC.getSelectedItem(), accNumTF.getText())) 
																						&& accountManager.remove(accNumTF.getText()) ) {
						JOptionPane.showMessageDialog(null, 
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					for (Account account : accOwnerList) {
						if (accTypeC.getSelectedItem().equals("전체계좌") || account.getAccType().equals(accTypeC.getSelectedItem())) {
							accListTA.append(account.toString()+"\n");
							cnt++;
						}
					}
		...
		registAccB.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if(isRegistable() && (!accLoanTF.isEnabled())) {
					try {
						accountManager.addAccount(accNumTF.getText(), 
										   		  accOwnerTF.getText(), 
					...
					try {
						accountManager.add(new MinusAccount(accNumTF.getText(), 
										   accOwnerTF.getText(), 
										   Integer.valueOf(accPwdTF.getText()), 
										   Long.valueOf(accDepositTF.getText()), 
