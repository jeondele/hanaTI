...
//         Enumeration<Client> e1 = chatRoom.getClientList().elements();
		...
		case Protocol.ROOM_USER_LIST:
			String roomUserListMsg = String.valueOf(Protocol.ROOM_USER_LIST);
			String clickedIdx = tokens[1];
			System.out.println(clickedIdx);
			Room clickedRoom = chatServer.getRoomList().get(Integer.valueOf(clickedIdx));
			Vector<Client> clickedClients = clickedRoom.getClientList();
			Enumeration<Client> clicked = clickedClients.elements();
			
			while(clicked.hasMoreElements()) {
				String nick = clicked.nextElement().nickName;
				roomUserListMsg += (Protocol.DELEMETER + nick + Protocol.DELEMETER + clickedIdx);
			}
			System.out.println(roomUserListMsg);
			sendMessage(roomUserListMsg);
			break;
